...
- Kết quả các lượt chạy:
  - **Lượt 0 (đã chạy thật, không cần API key)** — lưới an toàn xử lý bằng code (`codebase/test_safety_checks.py`): **10/10 test PASS** — lọc tin bot, lọc tin rác, đếm lại đúng số người duy nhất (D6014 hỏi 2 lần → tính đúng 1 người), che mã định danh, tính đúng mức ưu tiên, và **lưới an toàn riêng cho câu hỏi cá nhân** (xem phát hiện ở Lượt 2).
  ...
  - **Lượt 2 (chạy đủ golden set qua `eval/run_eval.py`, không lọc theo ngày, 766 tin → 127 cụm, trước khi vá)** — 4/5 case tự động PASS, 1 N/A. **Phát hiện quan trọng nhất: case ③ (M28943, câu hỏi cá nhân về điểm danh) FAIL — model đưa vào cụm công khai dù prompt đã cấm.** Đây là vi phạm quality bar (lớp ③ yêu cầu 100%). Đã vá bằng lưới an toàn thứ 2 độc lập với prompt (`strip_personal_messages` trong `codebase/cluster_report.py`, dùng heuristic từ khoá rà lại nội dung gốc) — có test tự động xác nhận (`test_strip_personal_messages_catches_model_mistake`, PASS).
  - **Lượt 3 (chạy lại sau khi vá, 18/9)** — **5/5 case tự động đạt** (1 N/A không tính), case ③ chuyển từ FAIL → **PASS** (xem `eval/run_log_2.md`, bản cập nhật). Xác nhận lưới an toàn hoạt động đúng trên dữ liệu thật. Bài học: không thể chỉ tin AI tuân theo instruction ở lớp lỗi nghiêm trọng nhất, phải có lưới an toàn bằng code độc lập — và phải tự kiểm tra lại chính bộ đo (lượt đầu tiên sau khi vá vẫn báo FAIL vì quên nối lưới an toàn vào script, xem lịch sử commit). 13 case thường/hiếm còn lại trong golden set cần D chấm tay bằng cách đọc trực tiếp output AI.
