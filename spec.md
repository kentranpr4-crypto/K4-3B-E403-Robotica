...
- Kết quả các lượt chạy:
  ...
  - **Lượt 2 (chạy đủ golden set qua `eval/run_eval.py`, không lọc theo ngày, 766 tin → 128 cụm)** — 4/5 case tự động PASS, 1 N/A (xem `eval/run_log_2.md`). **Phát hiện quan trọng nhất: case ③ (M28943, câu hỏi cá nhân về điểm danh) FAIL — model đưa vào cụm công khai dù prompt đã cấm.** Đây là vi phạm quality bar (lớp ③ yêu cầu 100%). Đã vá bằng lưới an toàn thứ 2 độc lập với prompt (`strip_personal_messages` trong `codebase/cluster_report.py`, dùng heuristic từ khoá rà lại nội dung gốc) — có test tự động xác nhận chặn đúng case này (`test_strip_personal_messages_catches_model_mistake`, PASS). Bài học: không thể chỉ tin AI tuân theo instruction ở lớp lỗi nghiêm trọng nhất, phải có lưới an toàn bằng code độc lập.
  - **Lượt 3 (chạy lại `run_eval.py` sau khi vá lưới an toàn cá nhân)** — [CẦN ĐIỀN — D chạy lại để xác nhận case 5 chuyển PASS, còn 13 case thường/hiếm khác cần D chấm tay bằng cách đọc output].
